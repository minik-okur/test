...
.uy-soru-metin {
  ...
  background: rgba(255,255,255,0.7); padding: 4px 16px;
  border-radius: 50px;
}

/* ── Roket Animasyon Alanı ── */
.uy-roket-alan {
...
.uy-roket.uy-zipladi {
  animation: uyRoketZipla 0.6s ease forwards;
}
...
.uy-roket.uy-sallandi {
  animation: uyRoketSalla 0.4s ease;
}
...
.uy-yol-track {
  ...
  height: 4px; background: rgba(255,255,255,0.3); border-radius: 4px;
}
.uy-yol-nokta {
...
.uy-bitis-overlay.gorунur { display: flex; animation: uyModalGel 0.35s ease; }
...
@keyframes gezegenParla {
  ...
  // ── Kelime Veritabanı (MEB 1. Sınıf) ────────────────────────────
  const GEZEGENLER = [
    {
      id: 'mars', adi: 'MARS', seviye: 1, renk: '#e53935',
      aciklama: 'Hayvanlar — Kolay',
      kelimeler: [
        { k: 'KÖPEK',  e: '🐕', secenekler: ['KÖPEK','KEDİ','KUŞU'] },
        { k: 'KEDİ',   e: '🐱', secenekler: ['KEDİ','KÖPEK','BALIK'] },
        { k: 'KUŞU',   e: '🦅', secenekler: ['KUŞU','KEDİ','AT'] },
        { k: 'ELMA',   e: '🍎', secenekler: ['ELMA','AYVA','ARMUDU'] },
        { k: 'BABA',   e: '👨', secenekler: ['BABA','ANA','KIZI'] },
      ]
    },
    {
      id: 'venus', adi: 'VENÜS', seviye: 2, renk: '#ff8f00',
      aciklama: 'Meyveler & Okul — Kolay',
      kelimeler: [
        { k: 'MUZA',   e: '🍌', secenekler: ['MUZA','KARPUZ','ELMA'] },
        { k: 'ÜZÜM',   e: '🍇', secenekler: ['ÜZÜM','MUZA','ELMA'] },
        { k: 'KİTAP',  e: '📚', secenekler: ['KİTAP','KALEM','DEFTER'] },
        { k: 'KALEM',  e: '✏️', secenekler: ['KALEM','KİTAP','SİLGİ'] },
        { k: 'DEFTER', e: '📓', secenekler: ['DEFTER','KİTAP','KALEM'] },
      ]
    },
    {
      id: 'jupiler', adi: 'JÜPİTER', seviye: 3, renk: '#1e88e5',
      aciklama: 'Hayvanlar & Yer — Orta',
      kelimeler: [
        { k: 'BALIK',  e: '🐟', secenekler: ['BALIK','TAVUK','KOYUN'] },
        { k: 'TAVUK',  e: '🐔', secenekler: ['TAVUK','BALIK','İNEK'] },
        { k: 'KOYUN',  e: '🐑', secenekler: ['KOYUN','TAVUK','AT'] },
        { k: 'ŞEHİR',  e: '🏙️', secenekler: ['ŞEHİR','KÖY','KASABA'] },
        { k: 'OKUL',   e: '🏫', secenekler: ['OKUL','EV','PARK'] },
      ]
    },
    {
      id: 'neptun', adi: 'NEPTÜN', seviye: 4, renk: '#00897b',
      aciklama: 'Fiiller & Kavramlar — Zor',
      kelimeler: [
        { k: 'DÜŞÜN',  e: '🤔', secenekler: ['DÜŞÜN','UYUR','OYNAR'] },
        { k: 'OYUN',   e: '🎮', secenekler: ['OYUN','OYUNCU','DERS'] },
        { k: 'HEDEF',  e: '🎯', secenekler: ['HEDEF','HEDİYE','HİKAYE'] },
        { k: 'RENK',   e: '🎨', secenekler: ['RENK','RESIM','BOYA'] },
        { k: 'MÜZİK',  e: '🎵', secenekler: ['MÜZİK','ŞARKI','SES'] },
      ]
    },
    {
      id: 'saturn', adi: 'SATÜRN', seviye: 5, renk: '#8e24aa',
      aciklama: 'Bonus Seviye — En Zor',
      kelimeler: [
        { k: 'ÇANTA',       e: '🎒', secenekler: ['ÇANTA','TORBA','KOVA'] },
        { k: 'PENCERE',     e: '🪟', secenekler: ['PENCERE','KAPI','DUVAR'] },
        { k: 'UÇAK',        e: '✈️', secenekler: ['UÇAK','GEMİ','TRENİ'] },
        { k: 'MÜZİSYEN',    e: '🎸', secenekler: ['MÜZİSYEN','MÜZIK','ŞARKI'] },
        { k: 'KÜTÜPHANE',   e: '📖', secenekler: ['KÜTÜPHANE','KİTABEVİ','OKUL'] },
      ]
    }
  ];
  ...
  // ── State ────────────────────────────────────────────────────────
  let state = {
    ekran: 'menu',         // 'menu' | 'oyun'
    aktifGezegen: null,
    soruIdx: 0,
    yanlisSayisi: 0,
    yanlisYapildi: false,
    kilit: false,
    ilerleme: {},          // { mars: 5, venus: 0, ... }
    acik: {}               // { mars: true, venus: false, ... }
  };

  function ilerlemeyiYukle() {
    try {
      const k = localStorage.getItem('uzayYolu_v2');
      if (k) {
    ...
    // İlk gezegen her zaman açık
    if (!state.acik['mars']) state.acik['mars'] = true;
  }

  function ilerlemeKaydet() {
    try {
      localStorage.setItem('uzayYolu_v2', JSON.stringify({
  ...
  function marsSVG(w) {
    return `<svg width="${w}" height="${w}" viewBox="0 0 60 60" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <radialGradient id="mgr" cx="38%" cy="32%">
          <stop offset="0%" stop-color="#ff7043"/>
          <stop offset="100%" stop-color="#b71c1c"/>
        </radialGradient>
      </defs>
      <circle cx="30" cy="30" r="26" fill="url(#mgr)"/>
      ...
      <circle cx="30" cy="30" r="26" fill="none" stroke="rgba(255,255,255,0.15)" stroke-width="1.5"/>
    </svg>`;
  }
  ...
  function venusSVG(w) {
    return `<svg width="${w}" height="${w}" viewBox="0 0 60 60" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <radialGradient id="vgr" cx="38%" cy="32%">
          <stop offset="0%" stop-color="#ffee58"/>
          <stop offset="100%" stop-color="#f57f17"/>
        </radialGradient>
      </defs>
      <circle cx="30" cy="30" r="26" fill="url(#vgr)"/>
      <ellipse cx="18" cy="26" rx="10" ry="4" fill="rgba(255,255,255,0.3)" transform="rotate(-15,18,26)"/>
      <ellipse cx="38" cy="34" rx="8" ry="3" fill="rgba(255,255,255,0.25)" transform="rotate(-10,38,34)"/>
      <ellipse cx="28" cy="40" rx="7" ry="2.5" fill="rgba(255,255,255,0.2)" transform="rotate(-5,28,40)"/>
      <circle cx="30" cy="30" r="26" fill="none" stroke="rgba(255,255,255,0.2)" stroke-width="1.5"/>
    </svg>`;
  }
  ...
  function jupiterSVG(w) {
    return `<svg width="${w}" height="${w}" viewBox="0 0 60 60" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <radialGradient id="jgr" cx="38%" cy="32%">
          <stop offset="0%" stop-color="#64b5f6"/>
          <stop offset="100%" stop-color="#0d47a1"/>
        </radialGradient>
        <clipPath id="jclip"><circle cx="30" cy="30" r="26"/></clipPath>
      </defs>
      <circle cx="30" cy="30" r="26" fill="url(#jgr)"/>
      <g clip-path="url(#jclip)">
        <ellipse cx="30" cy="22" rx="28" ry="3" fill="rgba(255,255,255,0.15)"/>
        <ellipse cx="30" cy="29" rx="28" ry="2.5" fill="rgba(255,255,255,0.1)"/>
        <ellipse cx="30" cy="36" rx="28" ry="3" fill="rgba(255,255,255,0.15)"/>
        <ellipse cx="30" cy="43" rx="28" ry="2" fill="rgba(255,255,255,0.1)"/>
        <ellipse cx="38" cy="29" rx="5" ry="4" fill="rgba(255,255,255,0.2)"/>
      </g>
      <circle cx="30" cy="30" r="26" fill="none" stroke="rgba(255,255,255,0.2)" stroke-width="1.5"/>
    </svg>`;
  }
  ...
  function neptunSVG(w) {
    return `<svg width="${w}" height="${w}" viewBox="0 0 60 60" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <radialGradient id="ngr" cx="38%" cy="32%">
          <stop offset="0%" stop-color="#4db6ac"/>
          <stop offset="100%" stop-color="#004d40"/>
        </radialGradient>
      </defs>
      <circle cx="30" cy="30" r="26" fill="url(#ngr)"/>
      <ellipse cx="26" cy="22" rx="8" ry="6" fill="rgba(255,255,255,0.12)" transform="rotate(-20,26,22)"/>
      <ellipse cx="36" cy="34" rx="6" ry="4" fill="rgba(255,255,255,0.1)" transform="rotate(15,36,34)"/>
      <circle cx="22" cy="38" r="4" fill="rgba(0,0,0,0.12)"/>
      <circle cx="30" cy="30" r="26" fill="none" stroke="rgba(255,255,255,0.2)" stroke-width="1.5"/>
    </svg>`;
  }

  function saturnSVG(w) {
    return `<svg width="${w}" height="${w}" viewBox="0 0 80 60" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <radialGradient id="sgr" cx="38%" cy="32%">
          <stop offset="0%" stop-color="#ce93d8"/>
          <stop offset="100%" stop-color="#4a148c"/>
        </radialGradient>
      </defs>
      <ellipse cx="40" cy="30" rx="40" ry="8" fill="rgba(206,147,216,0.25)" stroke="rgba(206,147,216,0.5)" stroke-width="2"/>
      <circle cx="40" cy="30" r="22" fill="url(#sgr)"/>
      <ellipse cx="40" cy="30" rx="38" ry="7" fill="none" stroke="rgba(206,147,216,0.6)" stroke-width="2.5"/>
      <circle cx="40" cy="30" r="22" fill="none" stroke="rgba(255,255,255,0.15)" stroke-width="1.5"/>
    </svg>`;
  }
  ...
  // ── Ses (global ses yöneticisini kullan, yoksa Web Audio) ─────────
  function sesCal(tip) {
    // Uygulamanın kendi ses sistemi varsa kullan
    if (window.sesleri && window.sesleri[tip]) {
      try { window.sesleri[tip].currentTime = 0; window.sesleri[tip].play(); } catch(e) {}
      return;
    }
    // Yoksa Web Audio API ile sentetik ses
    try {
      ...
      if (tip === 'dogru') {
        ...
        [523,659,784,1047].forEach((f,i) => {
          ...
          gb.gain.setValueAtTime(0.25, ctx.currentTime + i*0.12);
          gb.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + i*0.12 + 0.3);
          ob.start(ctx.currentTime + i*0.12);
          ob.stop(ctx.currentTime + i*0.12 + 0.3);
        });
      } else if (tip === 'gecis') {
        o.frequency.setValueAtTime(400, ctx.currentTime);
        o.frequency.linearRampToValueAtTime(800, ctx.currentTime + 0.3);
  ...
  // HTML'de uzayYoluScreen yoksa oluştur
  let screen = document.getElementById('uzayYoluScreen');
  if (!screen) {
    screen = document.createElement('div');
    screen.id = 'uzayYoluScreen';
  ...
  function menuRender() {
    ...
    let kartlarHTML = GEZEGENLER.map((g, idx) => {
      ...
      return `
        <div class="uy-gezegen-kart ${acik?'':'uy-kilitli'} ${tamamlandi?'uy-tamamlandi':''}"
          ...
          ${acik
            ? `<div style="font-size:1.3rem">${tamamlandi?'✅':'▶️'}</div>`
    ...
    alan.innerHTML = `
      <div class="uy-menu">
        ...
        <div class="uy-toplam-bar">
          ⭐ Toplam Yıldız: <strong>${toplamYildiz} / ${maxYildiz}</strong>
        </div>
      </div>`;
  ...
  function soruyuGoster() {
    ...
    roket.style.left = (8 + state.soruIdx * 17) + '%';

    // Seçenekler
    const karisikilmis = shuffle([...s.secenekler]);
    const konteyner = document.getElementById('uySecenekler');
    konteyner.innerHTML = '';
    karisikilmis.forEach(secenek => {
      const btn = document.createElement('button');
      btn.className = 'uy-sec-btn';
      btn.textContent = secenek;
      btn.setAttribute('aria-label', secenek + ' seçeneği');
  ...
  function cevapKontrol(secilen, btn) {
    ...
    if (secilen === s.k) {
      ...
      // YANLIŞ
      if (state.yanlisYapildi) return; // İkinci yanlışı engelle
  ...
  function gezegenBit() {
    ...
    let yildizHTML = '';
    for (let i = 0; i < 5; i++) {
      yildizHTML += `<span style="opacity:0;transition:opacity 0.3s ease ${i*0.2}s">${i < yildiz ? '⭐' : '☆'}</span>`;
    }

    const sonraki = sirIdx < GEZEGENLER.length - 1 ? GEZEGENLER[sirIdx+1] : null;

    let modalHTML = `
      <div class="uy-bitis-overlay gorунур" id="uyBitisOverlay">
        <div class="uy-bitis-kart">
          <div class="uy-bitis-emoji">🎉</div>
          ...
          <div class="uy-bitis-stat">
            ✅ Doğru: ${dogruSayisi}/5 &nbsp;&nbsp; ⭐ Yıldız: ${yildiz}
          </div>
    ...
    // Yıldız animasyonu
    setTimeout(() => {
      document.querySelectorAll('#uyBitisYildiz span').forEach(sp => {
        sp.style.opacity = '1';
      });
    }, 200);

    document.getElementById('uyBtnMenu').addEventListener('click', () => {
      document.getElementById('uyBitisOverlay').remove();
      menuRender();
    });

    document.getElementById('uyBtnSonraki').addEventListener('click', () => {
      document.getElementById('uyBitisOverlay').remove();
      if (sonraki) {
        sesCal('gecis');
